...
func NewRouter(db *sql.DB, cfg config.Config, web fs.FS, ss *screenshot.Service) http.Handler {
	...
	mux.Handle("POST /api/items/{id}/favorite", auth.RequireAuth(db, srv.writeErr, http.HandlerFunc(srv.FavoriteItem)))
	mux.Handle("DELETE /api/items/{id}/favorite", auth.RequireAuth(db, srv.writeErr, http.HandlerFunc(srv.UnfavoriteItem)))
